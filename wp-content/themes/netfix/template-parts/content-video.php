 ...
 */
    ...
    $netfix_video_url = function_exists( 'get_field' ) ? get_field( 'fromate_style' ) : NULL;
    $categories = get_the_terms( $post->ID, 'category' );
    if ( is_single() ): 
?>

    <article id="post-<?php the_ID();?>" <?php post_class( 'standard-blog-item mb-50 format-video' );?> data-wow-delay=".3s">
        <?php if ( has_post_thumbnail() ): ?>    
        <div class="standard-post-thumb video-thumb">
                <?php the_post_thumbnail( 'full', ['class' => 'img-responsive'] );?>
            <?php if(!empty($netfix_video_url)) : ?>
            <div class="blog__play">
                <a href="<?php print esc_url( $netfix_video_url );?>" class="btn-play popup-video-link"><i class="fal fa-play"></i></a>
            </div>
            <?php endif; ?>
        </div>
        <?php endif; ?>
